...
For some mobile apps, routing domains was not enough. Apps may try QUIC/HTTP3
over `UDP/443` and skip the domain-based routing path. With the tested
VLESS Reality / XTLS Vision outbound, routing `UDP/443` through VLESS caused
`XTLS rejected UDP/443 traffic`, so the working mitigation was to block
`UDP/443` and force apps back to TCP/443, where Xray can sniff domains and
...
- есть правила для Instagram и X/Twitter;
- есть фикс для мобильных приложений через блокировку `UDP/443`, чтобы форсить TCP fallback;
...
- Instagram or X/Twitter opens inconsistently through a router-level setup;
...
| Instagram does not load | Instagram and Meta assets use multiple domains, not only `instagram.com` | Route `instagram.com`, `cdninstagram.com`, `fbcdn.net`, `facebook.com`, `facebook.net`, `fbsbx.com`, `meta.com`, and `threads.net` through `vless-reality` |
| X / Twitter opens inconsistently | X uses separate API, short-link, media, and image CDN domains | Route `x.com`, `twitter.com`, `t.co`, `twimg.com`, `pbs.twimg.com`, `video.twimg.com`, and related Twitter domains through `vless-reality` |
| ChatGPT works in browser but mobile app says country is unavailable | Mobile app may use QUIC/HTTP3 over `UDP/443`, bypassing domain-based routing; the tested VLESS Reality / XTLS Vision outbound rejected UDP/443 | Block `UDP/443` so apps fall back to TCP/443 and Xray can route by domain |
...
Keenetic, Netcraze, Hopper, NC-3811, KN-3811, XKeen, Entware, OPKG, Xray, VLESS Reality, BlancVPN, YouTube, Smart TV, Telegram, Telegram media, Telegram CDN, Instagram, X, Twitter, X.com, Claude, ChatGPT, ChatGPT mobile, OpenAI, Kino.pub, Kino.watch, WOT Blitz, Tanks Blitz, QUIC, HTTP3, UDP 443, block UDP 443, router VPN, KeeneticOS, OpenWrt alternative, VPN на роутере, YouTube на телевизоре, YouTube на Smart TV, Instagram на Keenetic, Twitter на Keenetic, X.com на Keenetic, ChatGPT на телефоне, Telegram на Keenetic, не грузятся картинки Telegram, Telegram media через VPN, обход блокировок на Keenetic, настройка XKeen, BlancVPN на роутере, VLESS на роутере.
